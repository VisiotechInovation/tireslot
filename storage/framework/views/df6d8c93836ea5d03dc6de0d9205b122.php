...
<?php endif; ?>
 ...
 <h1 class="table--name"><?php echo e(__('Accounts')); ?> (<?php echo e($accounts->total()); ?>)</h1>
    ...
    <tr>
     ...
     <?php $__currentLoopData = $selectedColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <?php if($this->showColumn($column)): ?>
       <th <?php if($index > count($selectedColumns) - 12): ?> class="hidden" <?php endif; ?>>
   ...
   <tbody>
    ...
    <?php else: ?>
     <?php
      $i = 0;
     ?>
     <?php $__currentLoopData = $accounts; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $nr => $account): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
      <tr <?php if($loop->last): ?> id="last_record" <?php endif; ?>
       class="expandable-row <?php if($this->isChecked($account->id)): ?> active <?php endif; ?>">
       <td style="border-left: none" data-title="Check">
        <div class="checkbox--primary">
         <input type="checkbox" value="<?php echo e($account->id); ?>" id="<?php echo e($account->id); ?>" wire:model="checked">
         <label for="<?php echo e($account->id); ?>"></label>
        </div>
       </td>
       <?php $__currentLoopData = $selectedColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <td <?php if($index > count($selectedColumns) - 12): ?> class="hidden" <?php endif; ?> data-title="<?php echo e($column); ?>"
         wire:click="expandRow(<?php echo e($nr); ?>)">
         <?php if($column === 'name'): ?>
          <a href="<?php echo e(route('show_account', ['id' => $account->id])); ?>"><?php echo e($account->name); ?></a>
         <?php elseif($column === 'active' || $column === 'store_tab' || $column === 'has_parent'): ?>
          <?php if($account->$column): ?>
           true
          <?php else: ?>
           false
          <?php endif; ?>
         <?php else: ?>
          <?php echo e($account->$column); ?>

         <?php endif; ?>
        </td>
       <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
       <td style="border-right: none">
        <button wire:click.prevent="confirmItemRemoval(<?php echo e($account->id); ?>)"
        ...
        <div class="details">
         <?php $__currentLoopData = $selectedColumns; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $index => $column): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
          <?php if($index >= count($selectedColumns) - 11): ?>
           <p>
            <?php if(!empty($account->$column)): ?>
             <bold><?php echo e($column); ?>:</bold> <?php echo e($account->$column); ?>
...
<?php endif; ?>
  ...
  <?php if($loadAmount <= count($accounts)): ?>
...
<?php /**PATH D:\Cloud\Comps\5 - EMBIANZ\1 - Iosif\Embianz\resources\views/livewire/accountstable.blade.php ENDPATH**/ ?>